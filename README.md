...
一个 **LSPosed / Xposed** 模块（libxposed 现代 API 102），注入**小米运动健康**（`com.mi.health`）进程内，拦截小爱同学的语音问答链路，把小爱原本的回答替换为**任意大模型**的回复；支持多模型管理、手环语音切换模型、智能家居指令直通。

当前版本：**v0.7.2.1**（versionCode 7021，包名 `com.zeroone01.xiaoai`）
仓库地址：<https://github.com/FengQi0124/hook.xiaomi.health.xiaoai>

> 多模型切换菜单、提供方预设、语音切换模型、智能家居指令直通；0.5.1-beta1 起自绘应用图标（运动手表 + AI 火花，自适应图标），并移除「我的」页入口注入、「启用模块」开关与预测性返回开关。0.6.0-beta1 起配置页按功能重新分栏（模型列表 / 其它 / 配置）；0.7.2.1 起「模式持续时长」机制整体删除（语音切换长期生效），并默认收敛上下文以省 token。
...
- 「模型列表」分组第 1 行固定**小爱同学**（不可编辑/删除），第 2 行起用户模型行尾有一个「**三横**」手柄：**点按**该行**行内展开精简编辑界面**（只露 **显示名称 / Base URL / 模型名 / API Key** 四栏 + **删除 · 取消 · 保存**，带展开-收起动画、不弹任何浮层，再点一次收起；显示名称即模型菜单里显示的自定义名字，可随时改），**长按 0.5 秒**进入排序态（横线变主题色 + 震动反馈），行变成圆角主题色卡片、**上下拖动**跟手移动，松手吸附落位并持久化（落位时各行直接停在新槽位，不会先飞离再弹回）；
...
- 配置：**模型列表**（小爱同学 → 居中的「+ 添加模型」）→ **其它**（测试模型可用性 / 智能家居指令直通）→ **配置**（切换模型提示词 / 系统提示词，0.6.0-beta1 由原「回答模式」栏改名并把系统提示词从「生成参数」移入；「小爱/LLM 模式持续时长」两条输入框在 0.6.0-beta1 删除，0.7.2.1 把整个时长机制一并删掉）→ 生成参数（超时 / 最大 Token）→ 会话设置（会话模式 / 窗口 / 上下文条数，卡底有省 token 说明）；
- 记录：API 调用次数 / token 用量 / 柱状图 / 最近调用，可刷新与清除；
- 主题页：跟随系统 / 浅色 / 深色 + 毛玻璃、悬浮栏玻璃效果、页面缩放（0.5.1-beta1 起「预测性返回」开关已删除，返回手势走系统默认行为）。

### 省 token（0.7.2.1，默认就收敛）

API 按 token 计费，故每次请求能少发就少发：

- 默认**上下文 4 条**（`DEFAULT_CONTEXT_LENGTH`，原 10 条）：每条历史都是整轮问答原样回传，条数直接决定花费；
- 历史另有 **1200 字符预算**兜底（`HISTORY_CHAR_LIMIT`，`LlmClient` 二次裁剪）：即使把上下文条数调大，单次请求回传的历史也有成本上界；
- 「会话设置」选 `independent` **完全不带历史**，最省；`single` 只在 60 秒窗口内续接上下文；
- `max_tokens` 默认 200、系统提示词精简可编辑；智能家居指令、切模型指令**不调用模型**直接走原生链路。
...
- **0.7.2.1**（当前，versionCode 7021）：行内编辑界面补回**显示名称**一栏（用户自定义的模型名可随时修改，四栏：显示名称 / Base URL / 模型名 / API Key）；**「小爱/LLM 模式持续时长」当从没存在过** —— 配置键、默认值、`ConfigStore` 取值、`ModeState` 到期回退与「X 分钟后自动恢复」全部删除，语音切换后长期生效（宿主进程重启才回到设置页选定的默认模式）；**省 token**：默认上下文 10 → **4 条** + 回传历史 **1200 字符预算**兜底 + 会话设置卡加说明脚注；文档全面去掉上游工程出处的表述（含 CI artifact 名、`docs/` 内的旧源码路径）；删掉陈旧的 `docs/superpowers/` 规划文档（100KB+）以精简 Agent 上下文；版本号 `0.6.0-beta1`(600) → **`0.7.2.1`(7021)**。
- **0.6.0-beta1**（versionCode 600）：配置页按功能重新分栏 —— 「模型列表」（小爱同学 → 居中的「**+ 添加模型**」，Miuix `MiuixIcons.Add` + 只留四个字 + 整行水平居中，原箭头行与长说明删除）、「其它」（测试模型可用性 / 智能家居指令直通）、「**配置**」（原「回答模式」栏改名，**切换模型提示词 + 系统提示词**合并到同一栏，系统提示词从「生成参数」移入）；**删除「小爱模式持续时长」「LLM 模式持续时长」两条输入框**（当轮仅去 UI 入口，0.7.2.1 起整套时长机制删除）；版本号 `0.5.1-beta1`(501) → `0.6.0-beta1`(600)。
...
- **0.5.0-beta**：多模型菜单 / 提供方预设 / 语音切模型 / 智能家居直通 / 「我的」页入口注入 / 3-Tab 设置页。
