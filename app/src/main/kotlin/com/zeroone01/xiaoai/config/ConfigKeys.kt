...
object ConfigKeys {
    ...
    /** 模型列表 JSON（kotlinx 序列化的 List<ModelEntry>；不含第 1 行固定「小爱同学」） */
    const val KEY_MODEL_LIST = "model_list"

    /** 当前激活模型 id：特殊值 [VALUE_MODEL_XIAOAI]="xiaoai" 表示第 1 行小爱同学 */
    const val KEY_ACTIVE_MODEL = "active_model"

    /** 进入「语音选择模型」菜单的指令词库（换行分隔） */
    const val KEY_CMD_SWITCH_MODEL = "cmd_switch_model"

    /** 智能家居/设备控制指令直通小爱原生链路（不调 LLM） */
    const val KEY_SMART_HOME_PASS = "smart_home_pass"
    ...
    /** 模型昵称占位符：读取系统提示词时自动替换为当前激活模型的昵称（模型行的「显示名称」） */
    const val MODEL_NAME_PLACEHOLDER = "{<ModelName>}"

    /** 默认系统提示词：告知模型身份（{<ModelName>} 发送前替换为激活模型昵称）+ 手环语音场景约束 */
    const val DEFAULT_SYSTEM_PROMPT =
        "你是{<ModelName>}，通过小米手环回答用户问题。\n" +
        "由于用户是语音输入，可能会有些许错别字。\n" +
    ...
    // ---------- 多模型默认值（0.5.0-beta 新增） ----------

    /** 激活模型 id 的特殊值：第 1 行固定「小爱同学」（原生接管，不调 LLM） */
    const val VALUE_MODEL_XIAOAI = "xiaoai"

    /** 默认激活模型：小爱同学（与 DEFAULT_DEFAULT_MODE=llm 无关，首次添加模型后由用户切换） */
    const val DEFAULT_ACTIVE_MODEL = VALUE_MODEL_XIAOAI

    /** 进入模型选择菜单的默认指令词库（换行分隔） */
    const val DEFAULT_CMD_SWITCH_MODEL =
        "切换模型\n" +
        "换模型\n" +
        "选择模型\n" +
        "切换AI模型\n" +
    ...
    /** 智能家居指令直通：默认开启（设备控制走小爱原生链路，不调 LLM） */
    const val DEFAULT_SMART_HOME_PASS = true

    /** 语音选择模型菜单的有效时长（毫秒）：超时自动退出选择模式 */
    const val MODEL_MENU_TIMEOUT_MS = 30_000L
