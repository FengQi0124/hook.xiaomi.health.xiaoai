...
/**
 * 手环小爱AI 配置存储 —— 封装 Remote Preferences 的读写入口。
 *
...
class ConfigStore private constructor(private val prefs: SharedPreferences) {
    ...
    /** 语音切换的运行时模型覆盖（仅 Hook 进程内存态；App 端改选 active_model 时自动清除） */
    @Volatile
    private var runtimeModelId: String? = null

    /** 监听 active_model 变化：设置页改选模型后，以持久化值覆盖手环语音的临时切换 */
    private val activeModelWatcher =
        SharedPreferences.OnSharedPreferenceChangeListener { _, key ->
            if (key == ConfigKeys.KEY_ACTIVE_MODEL) runtimeModelId = null
        }

    init {
        prefs.registerOnSharedPreferenceChangeListener(activeModelWatcher)
    }
    ...
    /**
     * 模块启用开关 —— 「启用模块」开关已删除，模块恒为启用
     * （与 `isThinkingMode`/`isMiuixMonet` 同理：历史存储的 false 一律忽略）。
     */
    fun isEnabled(): Boolean = true

    /** API 类型："openai"（OpenAI 兼容）/"anthropic"；激活模型条目优先 */
    fun getApiType(): String {
        val entryType = getActiveModelEntry()?.apiType?.trim()
        if (!entryType.isNullOrEmpty()) return entryType
        return prefs.getString(ConfigKeys.KEY_API_TYPE, ConfigKeys.DEFAULT_API_TYPE)
            ?: ConfigKeys.DEFAULT_API_TYPE
    }

    /** 自定义请求地址：激活模型条目优先，空白或非法时回退默认值（避免残留脏值导致请求失败） */
    fun getBaseUrl(): String {
        val entryUrl = getActiveModelEntry()?.baseUrl?.trim()?.trimEnd('/')
        if (!entryUrl.isNullOrEmpty()) return entryUrl
    ...
    fun isAppendApiPath(): Boolean {
        getActiveModelEntry()?.let { return it.appendApiPath }
        return prefs.getBoolean(ConfigKeys.KEY_APPEND_API_PATH, ConfigKeys.DEFAULT_APPEND_API_PATH)
    }

    /** API Key：激活模型条目的 Key 优先（已解密），否则读全局 Key（读取时自动解密，解密失败返回空串） */
    fun getApiKey(): String {
        getActiveModelEntry()?.let { return it.apiKey }
        return ApiKeyCipher.decrypt(prefs.getString(ConfigKeys.KEY_API_KEY, "") ?: "")
    }

    /** 模型名称：激活模型条目优先 */
    fun getModel(): String {
        val entryModel = getActiveModelEntry()?.model?.trim()
        if (!entryModel.isNullOrEmpty()) return entryModel
        return prefs.getString(ConfigKeys.KEY_MODEL, ConfigKeys.DEFAULT_MODEL)
            ?: ConfigKeys.DEFAULT_MODEL
    }

    /**
     * 采样温度 / top_p / top_k 输入 UI 已删除（0.5.0-beta 第三轮精简），三个 getter
     * 恒返回 null —— LlmClient 据此不传这些参数，请求统一使用 API 默认值。
     * （与 [isThinkingMode] 的强制关断同理：历史存储值一律忽略。）
     */
    fun getTemperature(): Float? = null

    /** 同 [getTemperature]：UI 已删除，恒不传参 */
    fun getTopP(): Float? = null

    /** 同 [getTemperature]：UI 已删除，恒不传参 */
    fun getTopK(): Int? = null

    /**
     * 思考模式开关。
     * 0.5.0-beta 起该功能**已下线**（实测有 bug），强制返回 false —— 无论历史配置为何，
     * 请求一律走普通生成路径（LlmClient 中 thinking 相关分支因此恒不生效）。
     */
    fun isThinkingMode(): Boolean = false
    ...
    /**
     * 系统提示词**原始文本**（含 `{<ModelName>}` 占位符，供设置页编辑显示）。
     * 若用户未设置（空字符串），返回默认值。
     */
    fun getSystemPromptRaw(): String {
        val v = prefs.getString(ConfigKeys.KEY_SYSTEM_PROMPT, "") ?: ""
        return if (v.isBlank()) ConfigKeys.DEFAULT_SYSTEM_PROMPT else v
    }

    /**
     * 系统提示词（**发送前实际使用**）：含 [ConfigKeys.MODEL_NAME_PLACEHOLDER]（`{<ModelName>}`）
     * 时自动替换为当前激活模型的昵称（模型行「显示名称」），让模型「知道自己是谁」——
     * 未配置模型或昵称为空时回退模型名，再回退「小爱 AI」。
     */
    fun getSystemPrompt(): String {
        val raw = getSystemPromptRaw()
        if (!raw.contains(ConfigKeys.MODEL_NAME_PLACEHOLDER)) return raw
        val nick = getActiveModelEntry()?.name?.trim()?.takeIf { it.isNotEmpty() }
            ?: getModel().trim().takeIf { it.isNotEmpty() }
            ?: "小爱 AI"
        return raw.replace(ConfigKeys.MODEL_NAME_PLACEHOLDER, nick)
    }
     ...
     * 0.5.0-beta 起该功能入口已从设置页移除，**强制返回 false**（一律走激活模型的 LLM）。
     */
    fun getUsePhoneXiaoai(): Boolean = false
    ...
    /**
     * 读取模型列表（不含第 1 行固定「小爱同学」）。
     * 存储时 apiKey 已加密，此处逐条解密为明文；JSON 损坏时返回空列表（容错）。
     */
    fun getModelList(): List<ModelEntry> {
        val raw = prefs.getString(ConfigKeys.KEY_MODEL_LIST, "") ?: ""
        if (raw.isBlank()) return emptyList()
        return try {
            json.decodeFromString<List<ModelEntry>>(raw).map {
                it.copy(apiKey = ApiKeyCipher.decrypt(it.apiKey))
            }
        } catch (_: Throwable) {
            emptyList()
        }
    ...
    /** 写入模型列表：apiKey 逐条加密后落盘（明文不落盘） */
    fun setModelList(list: List<ModelEntry>) {
        val encoded = list.map { it.copy(apiKey = ApiKeyCipher.encrypt(it.apiKey)) }
        prefs.edit().putString(ConfigKeys.KEY_MODEL_LIST, json.encodeToString(encoded)).apply()
    }

    /** 当前激活模型 id：特殊值 [ConfigKeys.VALUE_MODEL_XIAOAI] 表示第 1 行小爱同学 */
    fun getActiveModelId(): String =
        prefs.getString(ConfigKeys.KEY_ACTIVE_MODEL, ConfigKeys.DEFAULT_ACTIVE_MODEL)
            ?: ConfigKeys.DEFAULT_ACTIVE_MODEL

    /** 写入激活模型 id（设置页改选行时调用；写入会同时清除语音运行时切换） */
    fun setActiveModelId(id: String) {
        prefs.edit().putString(ConfigKeys.KEY_ACTIVE_MODEL, id).apply()
    }

    /** 激活模型键是否已由设置页显式写入（用于首次迁移判断，避免删除全部模型后重复播种） */
    fun hasActiveModelKey(): Boolean = prefs.contains(ConfigKeys.KEY_ACTIVE_MODEL)

    /**
     * 设置语音切换的运行时模型覆盖（Hook 进程内存态）。
     * 传 null 表示清除覆盖、回到设置页持久化的激活模型。
     */
    fun setRuntimeModelId(id: String?) {
        runtimeModelId = id
    }

    /** 生效中的激活模型 id：语音运行时切换优先，其次持久化值 */
    fun getEffectiveModelId(): String = runtimeModelId ?: getActiveModelId()

    /**
     * 当前生效的 LLM 模型条目。
     * 选中「小爱同学」、或 id 不在列表中时返回 null（此时请求参数回退全局键值）。
     */
    fun getActiveModelEntry(): ModelEntry? {
        val id = getEffectiveModelId()
        if (id.isEmpty() || id == ConfigKeys.VALUE_MODEL_XIAOAI) return null
        return getModelList().firstOrNull { it.id == id }
    }

    /** 进入「语音选择模型」菜单的指令词库：规则同 [getCmdToLlm] */
    fun getCmdSwitchModel(): List<String> {
        val raw = prefs.getString(ConfigKeys.KEY_CMD_SWITCH_MODEL, "") ?: ""
        return splitCommandWords(raw).ifEmpty { splitCommandWords(ConfigKeys.DEFAULT_CMD_SWITCH_MODEL) }
    }

    /** 是否智能家居/设备控制指令直通小爱原生链路（不调 LLM）：默认开启 */
    fun getSmartHomePass(): Boolean =
        prefs.getBoolean(ConfigKeys.KEY_SMART_HOME_PASS, ConfigKeys.DEFAULT_SMART_HOME_PASS)

    // 「我的」页入口注入已整体移除：不再向宿主注入任何入口，打开设置页只走桌面图标。
    ...
    /**
     * Monet 动态取色开关 —— 0.5.0-beta 第三轮精简：取色 UI 已删除，恒返回 false
     * （themeMode 一律走非 Monet 系列，历史存储值忽略）。
     */
    fun isMiuixMonet(): Boolean = false
    ...
    /** 写入「语音选择模型」指令词库（多行文本存储，自动过滤空行） */
    fun setCmdSwitchModel(words: List<String>) {
        prefs.edit().putString(ConfigKeys.KEY_CMD_SWITCH_MODEL, words.joinToString("\n")).apply()
    }

    /** 设置是否智能家居指令直通小爱原生链路 */
    fun setSmartHomePass(v: Boolean) {
        prefs.edit().putBoolean(ConfigKeys.KEY_SMART_HOME_PASS, v).apply()
    }
